...
class VID_DCGAN(object):
    ...
    def generator(self, z, reuse=False, train=True):
        ...
        layers.gr0 = tf.nn.relu(linear(z_reshaped, 512, 'gvideo_0'))
        print "gr0:", layers.gr0.get_shape().as_list()
        layers.gr1 = tf.nn.relu(linear(layers.gr0, 512, 'gvideo_1'))
        print "gr1:", layers.gr1.get_shape().as_list()
        layers.gr2 = tf.nn.relu(linear(layers.gr1, 512, 'gvideo_2'))
