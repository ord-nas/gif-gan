...
class VID_DCGAN(object):
    ...
    def generator(self, z, reuse=False, train=True):
        ...
        layers.gr0 = tf.nn.relu(self.g_bn0(linear(z_reshaped, 256, 'gvideo_0')))
        print "gr0:", layers.gr0.get_shape().as_list()
        layers.gr1 = tf.nn.relu(self.g_bn1(linear(layers.gr0, 256, 'gvideo_1')))
        ...
        layers.gr3 = tf.nn.relu(self.g_bn3(linear(layers.gr2, 512, 'gvideo_3')))
        print "gr3:", layers.gr3.get_shape().as_list()
        layers.gr4 = tf.nn.tanh(linear(layers.gr3, self.z_output_size, 'gvideo_4'))
        print "gr4:", layers.gr4.get_shape().as_list()

        return layers.gr4, layers
