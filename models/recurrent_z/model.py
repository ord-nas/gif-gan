...
class DCGAN_with_mixed_batch(object):
    def __init__(self, sess, image_size=108, is_crop=True,
                 batch_size=64, sample_size = 64, output_size=64,
                 y_dim=None, z_dim=100, gf_dim=64, df_dim=64,
                 gfc_dim=1024, dfc_dim=1024, c_dim=3, dataset_name='default',
        ...
        assert y_dim is None, "Y input not supported!"
        self.z_dim = z_dim
    ...
    def build_model(self, z, sample_z):
        ...
        self.G = self.generator(self.z)
        (self.D, self.D_,
         self.D_logits, self.D_logits_,
         self.D_activations, self.D_activations_) = self.discriminator(self.images, self.G, reuse=False, train=True)

        # Batch norm in INFERENCE mode
        self.sampler = self.sampler(self.sample_z)
        (self.D_inf, self.D_inf_,
         self.D_logits_inf, self.D_logits_inf_,
         self.D_activations_inf, self.D_activations_inf_) = self.discriminator(self.images, self.sampler, reuse=True, train=False)
    ...
    def train(self, config):
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                errD_real = self.d_loss_real.eval({self.images: batch_images, self.z: batch_z})
    ...
    def discriminator(self, real_image, generated_image, reuse=False, train=True):
        if reuse:
            tf.get_variable_scope().reuse_variables()
